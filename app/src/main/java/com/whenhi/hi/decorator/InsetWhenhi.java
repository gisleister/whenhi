package com.whenhi.hi.decorator;
 ...
 * @since 07-19-2016
 */
public class InsetWhenhi extends RecyclerView.ItemDecoration {
    ...
    /**
     * Handy builder for creating {@link InsetWhenhi} instance.
     */
    public static class Builder {
        ...
        public InsetWhenhi build() {
            InsetWhenhi insetWhenhi = new InsetWhenhi();

            if (mDividerHeight == 0) {
                // Set default divider height to 1dp.
                insetWhenhi.setDividerHeight(mContext.getResources().getDimensionPixelSize(R.dimen.item_decoration_height));
            } else if (mDividerHeight > 0) {
                insetWhenhi.setDividerHeight(mDividerHeight);
            } else {
                throw new IllegalArgumentException("Divider's height can't be negative.");
            }

            insetWhenhi.setFirstInset(mFirstInset < 0 ? 0 : mFirstInset);
            insetWhenhi.setSecondInset(mSecondInset < 0 ? 0 : mSecondInset);

            if (mColor == 0) {
                throw new IllegalArgumentException("Don't forget to set color");
            } else {
                insetWhenhi.setColor(mColor);
            }

            if (mOrientation != InsetWhenhi.HORIZONTAL_LIST && mOrientation != InsetWhenhi.VERTICAL_LIST) {
                throw new IllegalArgumentException("Invalid orientation");
            } else {
                insetWhenhi.setOrientation(mOrientation);
            }

            insetWhenhi.setOverlay(mOverlay);
