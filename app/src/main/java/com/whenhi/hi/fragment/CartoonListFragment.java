 ...
 */
public class CartoonListFragment extends BaseFragment implements OnRefreshListener, OnLoadMoreListener,
    ...
    public void onLoadMore() {
        mPageNo++;
        HttpAPI.requestList("manhua", App.getExtras("manhua"),mPageNo, new HttpAPI.Callback<FeedModel>() {
            @Override
            public void onSuccess(FeedModel feedModel) {
                mSwipeToLoadLayout.setLoadingMore(false);
                if(feedModel.getState() == 0){
                    App.setExtras("manhua",feedModel.getData().getExtras());
