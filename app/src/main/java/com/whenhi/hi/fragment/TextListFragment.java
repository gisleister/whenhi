 ...
 */
public class TextListFragment extends BaseFragment implements OnRefreshListener, OnLoadMoreListener,
        OnItemClickListener<Feed>,
        OnItemLongClickListener<Feed> {
    private static final String TAG = TextListFragment.class.getSimpleName();
    ...
    public void onLoadMore() {
        mPageNo++;
        HttpAPI.requestList("text",App.getExtras("text"),mPageNo, new HttpAPI.Callback<FeedModel>() {
            @Override
            public void onSuccess(FeedModel feedModel) {
                mSwipeToLoadLayout.setLoadingMore(false);
                if(feedModel.getState() == 0){
                    App.setExtras("text",feedModel.getData().getExtras());
